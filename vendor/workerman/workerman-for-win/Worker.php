...
class Worker
{
    ...
    {
        // 保存worker实例
        $this->workerId = spl_object_hash($this);           //返回指定对象的hash id  ,对于每个对象它都是唯一的，并且对同一个对象它总是相同。
    ...
    public function listen()
    {
        ...
        // Non blocking.
        stream_set_blocking($this->_mainSocket, 0);         //为资源流设置阻塞或者阻塞模式  0，资源流将会被转换为非阻塞模式；如果是1，资源流将会被转换为阻塞模式
    ...
    protected static function getEventLoopName()
    {
        ...
        foreach (self::$_availableEventLoops as $name=>$class) {
            if (extension_loaded($name)) {      //检查一个扩展是否已经加载
                $loop_name = $name;
                break;
            }
        }
